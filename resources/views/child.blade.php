<x-guest-layout>
    <div class="mb-4 text-sm text-gray-600">
        <h2 class="text-lg font-medium text-gray-900">お子さまの情報登録 ({{ $siblingOrder }}人目)</h2>
        <p>お子さまの基本情報を入力してください。</p>
    </div>

    <form method="POST" action="{{ route('surveys.child.submit') }}">
        @if ($errors->any())
    ...
    @endif
        ...
        <div class="mt-4">
            ...
            <x-text-input id="birth_date" class="block mt-1 w-full" type="date" name="birth_date" :value="old('birth_date', '2010-01-01')" required />
        ...
        <div class="mt-4">
            ...
            <div class="mt-2 space-y-2">
                <div class="flex items-center">
                    <input id="gender_male" name="gender" type="radio" value="male" class="h-4 w-4 text-indigo-600 border-gray-300 focus:ring-indigo-500" {{ old('gender') == 'male' ? 'checked' : '' }} required>
                    <label for="gender_male" class="ml-3 block text-sm font-medium text-gray-700">男性</label>
                </div>
                <div class="flex items-center">
                    <input id="gender_female" name="gender" type="radio" value="female" class="h-4 w-4 text-indigo-600 border-gray-300 focus:ring-indigo-500" {{ old('gender') == 'female' ? 'checked' : '' }}>
                    <label for="gender_female" class="ml-3 block text-sm font-medium text-gray-700">女性</label>
                </div>
                <div class="flex items-center">
                    <input id="gender_other" name="gender" type="radio" value="other" class="h-4 w-4 text-indigo-600 border-gray-300 focus:ring-indigo-500" {{ old('gender') == 'other' ? 'checked' : '' }}>
        ...
        <div class="mt-4">
            ...
            <div class="mt-2 space-y-2">
                <div class="flex items-center">
                    <input id="menstruation_tracking_yes" name="menstruation_tracking" type="radio" value="1" class="h-4 w-4 text-indigo-600 border-gray-300 focus:ring-indigo-500" {{ old('menstruation_tracking') == '1' ? 'checked' : '' }} required>
                    <label for="menstruation_tracking_yes" class="ml-3 block text-sm font-medium text-gray-700">はい</label>
                </div>
                <div class="flex items-center">
                    <input id="menstruation_tracking_no" name="menstruation_tracking" type="radio" value="0" class="h-4 w-4 text-indigo-600 border-gray-300 focus:ring-indigo-500" {{ old('menstruation_tracking') == '0' ? 'checked' : '' }}>
        ...
        <div id="last_period_date_section" class="mt-4 hidden">
            <x-input-label for="last_period_date" value="最近の月経開始日" />
            <x-text-input id="last_period_date" class="block mt-1 w-full" type="date" name="last_period_date" :value="old('last_period_date')" />
            <div class="mt-2">
                <input id="input_later" type="checkbox" class="h-4 w-4 text-indigo-600 border-gray-300 focus:ring-indigo-500">
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (registerMenstrualCycleYes.checked) {
                lastPeriodDateSection.classList.remove('hidden');
            }
            ...
            registerMenstrualCycleNo.addEventListener('change', function() {
                if (this.checked) {
                    lastPeriodDateSection.classList.add('hidden');
